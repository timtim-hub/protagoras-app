	...
	<div class="row justify-content-center">	
		<div class="col-sm-12 text-center">
			...
			<div class="mb-4" id="balance-status">
				<span class="fs-11 text-muted pl-3"><i class="fa-sharp fa-solid fa-bolt-lightning mr-2 text-primary"></i>{{ __('Your Balance is') }} <span class="font-weight-semibold" id="balance-number1">@if (auth()->user()->gpt_4_turbo_credits == -1) {{ __('Unlimited') }} @else {{ number_format(auth()->user()->gpt_4_turbo_credits + auth()->user()->gpt_4_turbo_credits_prepaid) }} Protagoras SEO AI™ {{ __('Words') }} @endif</span></span>
			</div>
		</div>
										...
										<select id="model" name="model" class="form-select">
											...
											@foreach ($models as $model)
												...
												@elseif (trim($model) == 'gpt-4-0125-preview')
													<option value="{{ trim($model) }}">Protagoras SEO AI™</option>
									...
									<div id="form-group">
										...
										<select id="creativity" name="creativity" class="form-select">
											@if (!is_null($wizard['creativity']))
												<option value=0.25 @if ($wizard['creativity'] == 0) selected @endif>{{ __('Repetitive') }}</option>
												<option value=0.5 @if ($wizard['creativity'] == 0.25) selected @endif> {{ __('Deterministic') }}</option>																															
												<option value=0.8 @if ($wizard['creativity'] == 0.5) selected @endif> {{ __('Original') }}</option>																															
												<option value=0.9 @if ($wizard['creativity'] == 0.75) selected @endif> {{ __('Creative') }}</option>																															
												<option value=1 @if ($wizard['creativity'] == 1) selected @endif> {{ __('Imaginative') }}</option>		
											@else
												<option value=0>{{ __('Repetitive') }}</option>
												<option value=0.5> {{ __('Deterministic') }}</option>																															
												<option value=0.8> {{ __('Original') }}</option>																															
												<option value=0.9> {{ __('Creative') }}</option>																															
				...
				<div id="final-wizard-status" class="hidden">
					<div class="row justify-content-center mt-6">
						<div class="col-sm-12 text-center">
							...
							<h4 class="fs-22 font-weight-bold" id="result-processing"><span class="main-text">{{ __('Generating the article...') }}</span><span class="processing-time">It can take up to 5 minutes</span></h4>
