...
class ChatCSVController extends Controller
{
    ...
    {
        $chats = ChatSpecial::where('user_id', auth()->user()->id)->where('type', 'csv')->orderBy('created_at', 'desc')->get();
        $first_chat = ChatSpecial::where('user_id', auth()->user()->id)->where('type', 'csv')->first();
        $chat_code = ($first_chat) ? $first_chat->id : 'new';
        $prompts = ChatPrompt::all();

        if (auth()->user()->group == 'user') {
            if (config('settings.chat_csv_user_access') != 'allow') {
                toastr()->warning(__('AI Chat CSV feature is not available for free tier users, subscribe to get a proper access'));
                return redirect()->route('user.plans');
            } else {
                $limit = config('settings.chat_csv_file_size_user');
                return view('user.chat_csv.index', compact('chats', 'chat_code', 'limit', 'prompts'));
            }
        } elseif (auth()->user()->group == 'subscriber') {
            $plan = SubscriptionPlan::where('id', auth()->user()->plan_id)->first();
            if ($plan->chat_csv_feature == false) {     
                toastr()->warning(__('Your current subscription plan does not include support for AI Chat CSV feature'));
                return redirect()->back();                   
            } else {
                $limit = $plan->chat_csv_file_size;
                return view('user.chat_csv.index', compact('chats', 'chat_code', 'limit', 'prompts'));
            }
        } else {
            $limit = config('settings.chat_csv_file_size_user');
            return view('user.chat_csv.index', compact('chats', 'chat_code', 'prompts', 'limit'));
        }
