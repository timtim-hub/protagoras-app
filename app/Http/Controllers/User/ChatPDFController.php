...
class ChatPDFController extends Controller
{
    ...
    {
        $chats = ChatSpecial::where('user_id', auth()->user()->id)->where('type', 'pdf')->orderBy('created_at', 'desc')->get();
        $first_chat = ChatSpecial::where('user_id', auth()->user()->id)->where('type', 'pdf')->first();
        $chat_code = ($first_chat) ? $first_chat->id : 'new';
        $prompts = ChatPrompt::all();

        if (auth()->user()->group == 'user') {
            if (config('settings.chat_pdf_user_access') != 'allow') {
                toastr()->warning(__('AI Chat PDF feature is not available for free tier users, subscribe to get a proper access'));
                return redirect()->route('user.plans');
            } else {
                $limit = config('settings.chat_pdf_file_size_user');
                return view('user.chat_pdf.index', compact('chats', 'chat_code', 'limit', 'prompts'));
            }
        } elseif (auth()->user()->group == 'subscriber') {
            $plan = SubscriptionPlan::where('id', auth()->user()->plan_id)->first();
            if ($plan->chat_pdf_feature == false) {     
                toastr()->warning(__('Your current subscription plan does not include support for AI Chat PDF feature'));
                return redirect()->back();                   
            } else {
                $limit = $plan->chat_pdf_file_size;
                return view('user.chat_pdf.index', compact('chats', 'chat_code', 'limit', 'prompts'));
            }
        } else {
            $limit = config('settings.chat_pdf_file_size_user');
            return view('user.chat_pdf.index', compact('chats', 'chat_code', 'prompts', 'limit'));
        }
