	...
	<div class="row justify-content-center">	
		<div class="col-sm-12 text-center">
			...
			<div class="mb-4" id="balance-status">
				<span class="fs-11 text-muted pl-3"><i class="fa-sharp fa-solid fa-bolt-lightning mr-2 text-primary"></i><?php echo e(__('Your Balance is')); ?> <span class="font-weight-semibold" id="balance-number1"><?php if(auth()->user()->gpt_4_turbo_credits == -1): ?> <?php echo e(__('Unlimited')); ?> <?php else: ?> <?php echo e(number_format(auth()->user()->gpt_4_turbo_credits + auth()->user()->gpt_4_turbo_credits_prepaid)); ?> Protagoras SEO AI™ <?php echo e(__('Words')); ?> <?php endif; ?></span></span>
			</div>
		</div>
		...
		<div class="row justify-content-center">
			<div class="col-lg-4 col-md-12 col-sm-12">
				<div class="card border-0" id="all-wizard-options">
					...
					<!-- add a checkbox "Enable internet access"-->
						...
						<div class="row">						
							...
							<div class="col-sm-12 step-one">								
								<div class="input-box">	
									<h6 class="fs-11 mb-2 font-weight-semibold">Additional information for the text</h6>								
									<div class="form-group">						  
										<textarea type="text" rows=5 class="form-control" id="additional_information" name="additional_information" placeholder="Additional information to keep in mind..."></textarea>								 
									</div> 
								</div> 
							</div>
							...
							<div class="col-sm-12 step-one">
								<div class="input-box">
									<h6 class="fs-11 mb-2 font-weight-semibold">Enable Internet Access</h6>
									<input class="form-check-input" type="checkbox" id="internet-access" name="internet_access" checked>
								</div>
							</div>
										...
										<select id="model" name="model" class="form-select">
											...
											<?php $__currentLoopData = $models; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $model): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
												<?php if(trim($model) == 'gpt-4'): ?>
													<option value="<?php echo e(trim($model)); ?>"><?php echo e(__('GPT 4')); ?></option>
												<?php elseif(trim($model) == 'gpt-4-0125-preview'): ?>
													<option value="<?php echo e(trim($model)); ?>">Protagoras SEO AI™</option>
												<?php elseif(trim($model) == 'gpt-4-turbo-2024-04-09'): ?>
													<option value="<?php echo e(trim($model)); ?>"><?php echo e(__('GPT 4 Turbo with Vision')); ?></option>
												<?php else: ?>
									...
									<div id="form-group">
										...
										<select id="creativity" name="creativity" class="form-select">
											<?php if(!is_null($wizard['creativity'])): ?>
												<option value=0.25 <?php if($wizard['creativity'] == 0): ?> selected <?php endif; ?>><?php echo e(__('Repetitive')); ?></option>
												<option value=0.5 <?php if($wizard['creativity'] == 0.25): ?> selected <?php endif; ?>> <?php echo e(__('Deterministic')); ?></option>																															
												<option value=0.8 <?php if($wizard['creativity'] == 0.5): ?> selected <?php endif; ?>> <?php echo e(__('Original')); ?></option>																															
												<option value=0.9 <?php if($wizard['creativity'] == 0.75): ?> selected <?php endif; ?>> <?php echo e(__('Creative')); ?></option>																															
											...
											<?php else: ?>
												...
												<option value=0.5> <?php echo e(__('Deterministic')); ?></option>																															
												<option value=0.8> <?php echo e(__('Original')); ?></option>																															
												<option value=0.9> <?php echo e(__('Creative')); ?></option>																															
				...
				<div id="final-wizard-status" class="hidden">
					<div class="row justify-content-center mt-6">
						<div class="col-sm-12 text-center">
							...
							<h4 class="fs-22 font-weight-bold" id="result-processing"><span class="main-text"><?php echo e(__('Generating the article...')); ?></span><span class="processing-time">It can take up to 5 minutes</span></h4>
...
<script type="text/javascript">
	...
	$(function () {
		...
		//Internet Access
		$('#internet-access').on('change', function() {
    		let form = new FormData(document.getElementById('wizard-form'));
    		form.append('wizard', wizard_id);

    		// Überprüfe, ob das Kontrollkästchen für den Internetzugang aktiviert ist
    		let internetAccess = this.checked ? 1 : 0;

    		// Füge den Wert des Internetzugangs nur dann zum FormData-Objekt hinzu, wenn das Kontrollkästchen aktiviert ist
    		if (this.checked) {
    		    form.append('internet_access', internetAccess);
    		}
		
    		// Führe den AJAX-Aufruf nur dann durch, wenn das Kontrollkästchen aktiviert ist oder deaktiviert wird
    		$.ajax({
    		    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
    		    method: 'POST',
		...
		$('#generate-ideas').on('click', function(e) {
			...
			let form = new FormData(document.getElementById('wizard-form'));
			form.append('wizard', wizard_id);
			//form.append('additional_information', $('#additional_information').val());
...
<!--Add cool and modern blockquote styling.-->
<style>
	.blockquote {
	...
	.tbody, td, th , tfoot, thead, tr, table {
    ...
    0%, 100% { opacity: 1; }  /* The text is fully visible at the start and end of the animation. */
    50% { opacity: 0.2; }     /* The text's opacity reduces to 0.5 at the middle of the animation. */
	}
	...
	.processing-time {
    font-size: 14px;  /* Smaller than the main text */
    font-weight: normal;  /* Regular font weight */
    display: block;  /* Makes the span appear on a new line */
    margin-top: 4px;  /* Adds a little space above the span */
    color: #666;  /* Optionally, a lighter color to differentiate from the main text */
	}
</style>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/aihowgkq/protagoras.app/resources/views/user/templates/wizard/index.blade.php ENDPATH**/ ?>